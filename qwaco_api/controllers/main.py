...
import odoo
from odoo import http, SUPERUSER_ID, models, fields, _
...
def check_permissions(func):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        uid = request.env['res.users'].sudo().browse(access_token_data['user_id'])
        if not uid or not uid.active:
            error_descrip = _("Tài khoản không tồn tại !")
            error = '01'
            return error_response(400, error, error_descrip)
        request.session.uid = access_token_data['user_id']
        # Set session UID from current access token
        request.env.uid = access_token_data['user_id']
...
def get_user_id():
    user_id = request.env['ir.config_parameter'].sudo().get_param('rest_api.mobile_app_user')
    if not user_id:
