...
class SaleListReport(models.TransientModel):
    ...
    def action_report(self):
        ...
        for r in recs:
            ...
            ws.cell(row, 6).value, ws.cell(row, 6).style = '%s%s%s%s%s' % (r['xom_kh']['en_US']+',',
                                                    r['xa_kh']['en_US'] + ',',
                                                    r['huyen_kh']['en_US'] + ',',
                                                    r['tinh_kh']+',',
                                                    r['quoc_gia_kh']['vi_VN'] + ','),highlight
            ws.cell(row, 7).value, ws.cell(row, 7).style = r['ma_hd'],highlight
            ws.cell(row, 8).value, ws.cell(row, 8).style = r['ma_dhn'],highlight
            ws.cell(row, 9).value, ws.cell(row, 9).style = '%s%s%s%s%s' % (r['xom_kh']['en_US']+',',
                                                    r['xa_kh']['en_US'] + ',',
                                                    r['huyen_kh']['en_US'] + ',',
                                                    r['tinh_kh']+',',
                                                    r['quoc_gia_kh']['vi_VN']),highlight
